...
function GetExpression(ic){// get an previously stored search expression and copy to the search form
	...
	msgwin.document.writeln("<p><select  name=ex onchange=window.opener.stored=document.forma1.ex.options[document.forma1.ex.selectedIndex].value;window.opener.CopyExpr();self.close()>\n")
...
//	for ($i=0;$i<count($camposbusqueda);$i++){
//		echo "<OPTION value=\"---\">---";
//			$select=$camposbusqueda[$i];
//			$l=explode('|',$linea);
//		$parte4=$l[3];
//			$parte3=$l[2];
//			$parte2=$l[1];
//			$parte1=$l[0];
//			echo "<OPTION value=".$parte4.$asel;
//			if ($i==$jx) echo " selected ";
			
//			$select = file_get_contents($db_path.$arrHttp["base"]."/fsts/".$_SESSION["lang"]."/".$parte4);
// $lines = explode("\n", $select);
